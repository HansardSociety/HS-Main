...
const searchDbPath = `../../build/${ ENV }/db/search.json`;
let searchDb = require(searchDbPath);
//
...
function compileSearchData (dir, fileNames, callback) {
  fileNames.map(function(path) {
    ...
    ************************************************************/
    ...
    pageObj.keywords = mergedKeywordsPhrases.slice(0, 30); // Limit to top 20
...
fsx.writeFileSync(searchDbPath, JSON.stringify(searchDb))
